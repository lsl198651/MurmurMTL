...
def get_pregnancy_status(data):
    ...
    for l in data.split('\n'):
        if l.startswith('#Pregnancy status:'):
            try:
                is_pregnant = bool(sanitize_binary_value(
                    l.split(': ')[1].strip()))
            except:
                pass
    return is_pregnant
...
def get_systolic_murmur_timing(data):
    is_pregnant = None
    for l in data.split('\n'):
        if l.startswith('#Systolic murmur timing:'):
            try:
                is_pregnant = l.split(': ')[1]
            except:
                pass
    return is_pregnant


def get_diastolic_murmur_timing(data):
    is_pregnant = None
    for l in data.split('\n'):
        if l.startswith('#Diastolic murmur timing:'):
            try:
                is_pregnant = l.split(': ')[1]
            except:
                pass
    ...
    with open(filename, 'r') as f:
        for i, l in enumerate(f):
            if i == 0:
                ...
                labels = tuple(sanitize_binary_value(entry)
                               for entry in l.split(','))
            elif i == 3:
                probabilities = tuple(sanitize_scalar_value(entry)
                                      for entry in l.split(','))
            else:
                break
